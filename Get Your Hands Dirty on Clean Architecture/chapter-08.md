...
도메인 모델 그 자체만으로 애플리케이션 계층 내 서비스에서 서비스를 접근할 수 있는 풍부한 행위를 구현할 수 있다. 만일 우리가 도메인 객체를 외부 계층으로 전달하고 싶다면 매핑 없이 그렇게 할 수 있다. 왜냐하면 도메인 객체는 내/외부 포트에서 기대하는 상태 인터페이스를 구현하기 때문이다.

외부 계층은 인터페이스와 매핑할 지 혹은 자신의 모델로 매핑할지를 결정할 수 있다. 행위 수정이 상태 인터페이스에 노출되지 않기 때문에 도메인 객체의 상태를 무의식적으로 수정할 수 없다.

외부 계층에서 애플리케이션 계층으로 전달하는 객체는 또한 상태 인터페이스를 구현한다. 그리고 나서 애플리케이션 계층은 그 행위에 접근하기 위해서 그것을 실제 도메인 모델로 매핑해야 한다. 이러한 매핑은 DDD의 팩토리와 잘 맞아 떨어진다. DDD에서의 팩토리는 특정 상태의 도메인 객체를 재구성하는데 책임이 있고 실제 우리가 하고 있는 것과 같다. (*Domain-Driven Design by Eric Evans, 2004*)

매핑의 책임은 분명하다: 만일 하나의 계층이 다른 계층에서 객체를 수신하면, 우리는 그것을 그 계층에서 동작하는 어떤 것과 메핑한다. 그러므로 각 계층은  단방향으로만 매핑하고 이것을 "one-way" 매핑 전략이라고 한다.

그러나, 계층 간 매핑이 되기 때문에, 이 전략은 개념적으로 다른 전략보다 더 어렵다.

만일 레이어간 모델이 유사하다면, 이 전략은 더욱 힘을 발휘한다. 예를 들어, 읽기 전용 동작일 때, 필요한 모든 정보를 상태 인터페이스가 제공하기 때문에 웹 계층은 모델로 전혀 매핑할 필요가 없다. 
...
각 매핑 전략이 각기 다른 장단점을 가지기 때문에 전체 코드에서 사용되는 고정 글로벌 규칙으로 단일 전략으로 사용해서는 안될 것이다. 동일 코드에서 패턴을 섞어쓰는것은 일관성이 없어 보이기 때문에, 우리의 본능에 역행하는 행위이다. 그러나 그러한 패턴을 특정 경우에 적합하지 않고 단지 일관성을 위해서 선택하는 것은 무책임한 것이고 잘못된 것이다.

또한, 소프트웨어는 시간이 지남에 따라 진화하기 때문에, 어제 그 상황에서 선택한 전략이 오늘 최선의 선택이 아닐 수 있다. 고정 매핑 전략으로 시작하여 계속 유지하는 대신 - 코드를 빠르게 진화할 수 있는 단순 전략으로 시작하고 나중에 계층간 결합도를 더 낮울 수 있는 복잡한 전략으로 갈아탈 수도 있다.
...
예를 들어 쿼리로 조회하는 것 보다 유스케이스를 수정하는데에 대한 다른 매핑 가이드라인을 정의할 수도 있다. 또한 웹과 애플리케이션 계층 간, 그리고 애플리케이션과 영속성 계층 간에 다른 매핑전략을 사용할 수도 있다.

이러한 경우에 대한 가이드라인은 이와 같을 것이다.

만일 우리가 *유스케이스 수정 작업*을 하고 있다면, "full mapping" 전략이 유스케이스의 결합도를 낮추기 위해 웹과 애플리케이션 계층 사이에는 첫번째 선택이다. 이것은 유스케이스당 유효성 검증 규칙을 명확하게 하고 있고 어떤 유스케이스에서 불필요한 필드를 처리할 필요가 없을 것이다.

만일 우리가 유스케이스를 수정하는 작업을 하고 있다면, 매핑 오버헤드 없이 코드를 빠르게 변화할 수 있도록. "no mappping" 전략이 애플리케이션과 영속성 계층 사이에 첫번째 선택이다. 하지만, 애플리케이션 계층에 영속성 이슈를 다뤄야 하는 한, 우리는 영속성 계층에 영속성 이슈를 유지하기 위해서 "two-way" 매핑 전략으로 바꿀 수도 있다.
...
이 같은 가이드라인을 성공적으로 적용하기 위해서, 이런 것들이 개발자의 마음속에 있어야 한다. 그래서, 가이드라인이 논의 되어야 하고 팀내의 노력으로 지속적으로 수정 되어야 한다.



## How Does This Help Me Build Maintainable Software?

애플리케이션 계층 사이의 대문처럼 동작하는 내외부 포트로, 계층들이 서로 어떻게 통신하고 계층간 어떻게 매핑해야 할지를 살펴보았다.

각 유스케이스에 좁은 범위의 포트로, 우리는 유스케이스마다 다른 매핑 전략을 선택할 수 있고 시간이 지남에 따라 다른 유스케이스에 영향을 주지 않을 수 있고 어느 시점에 최선을 전략을 선택하면서 진화할 수 있다.

상황에 맞는 이러한 매핑 전략 선택과정은 모든 상황에 대한 동일 매핑전략을 사용하는 것보다 더 힘들고 많은 의사소통이 필요하다. 하지만 매핑 가이드라인이 정해지만 코드상 팀내에서 보상받을 수 있고 실행 및 유지보수하기 더 쉬워진다.
