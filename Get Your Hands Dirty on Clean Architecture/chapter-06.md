...
*1장(What's Wrong with Layers)*에서 전통적인 계층형 구조의 문제점에 대해 설명했고 결국 모든 것이 영속성 계층에 의존하기 때문에 "데이터베이스 주도 설계"를 유도한다고 했다. 이번 장에서는 영속성 계층의 의존성을 역전시켜서 애플리케이션 계층으로 플러그인하는 방식에 대한 내용을 살펴볼 것이다.
...
예를 들어 다음 그림에 나타난 대로 영속성 동작(DDD에서 aggregate)이 필요한 도메인 클래스당 하나의 영속성 어댑터를 구현하는 방법을 선택할 수도 있다. 
