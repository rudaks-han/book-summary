...
이 책 서문에서, 우리는 갚지 못하는 많은 기술부채만 쌓여가게 하는 응급처치로 항상 처리할 수밖에 없는 상황에 대한 불만을 얘기했다.

응급처치로 처리하지 않기 위해서 우리는 그 내용을 확인할 수 있어야 한다. 그래서 이 장의 목표는 잠재적인 응급처치에 대한 의식을 높이고 그 효과를 논의해보는 것이다.

이 정보로, 우연히 발생하는 응급처치를 인지하고 고쳐 나갈 수 있다. <u>만일 당연하다면 임시방편 작업의 효과로 의식적으로 참여할 수도 있다.</u>

건축공학 책이나 혹은 더욱 끔직한 것은 항공기술관련 책에서 앞에서 기술된 문장을 생각해봐라. 그러나 우리 대부분은 고층건물 혹은 항공기 관련 소프트웨어를 만들지는 않는다. 소프트웨어는 하드웨어보다 더 부드러워야 하고 변경이 쉬워야 한다. 그래서 우선 응급처치로 처리하고 나중에(결코 하지 않겠지만) 수정하는 것이 더 경제적일 수는 있다.



## 응급처치가 깨진 유리창과 같은 이유

1969년에 심리학자 Philip Zimbardo는 하나의 실험을 진행했고 후에 깨진 유리창 이론(https://www.theatlantic.com/ideastour/archive/windows.html)으로 알려지게 되었다.

그는 번호판 없는 자동차를 브론스 근처에 한대 주차했고 살기 더 좋은 곳으로 알려진 팔로 알토에 또 다른 한대를 주차했다. 그리고 기다렸다.

브론스에 있는 차는 24시간 내에 귀중품이 도난되었고 지나가는 사람들이 그 차를 부수기 시작했다. 

팔로 알토에 있는 차는 일주일 동안은 누구도 건드리지 않았서 Zimbardo는 창문을 깨뜨렸다. 그때 부터 차는 브론스의 차와 비슷하게 되었고 지나가는 사람들에 의해 금방 부서졌다.

차의 물건을 훔치거나 부수는 사람들은 모든 사회 계층 사람들과 법을 준수하고 선량한 시민들이었다.

이런 인간의 행동은 깨진 유리창 법칙 이론으로 알려져 있다. 

> 형편없고 훼손되거나 내버려둔 어떤 것이 있다면 사람의 뇌는 더 형편없고 훼손되게 한다.
...
* 많은 위반이 있는 코드로 작업할 때 추가되는 코드의 위반은 높아진다.
* 많은 응급처치가 있는 코드로 작업할 때 응급처치가 추가될 가능성은 높아진다.
...
코딩하는 것이 차를 훼손하는 것과 같지는 않지만 우리는 깨진 유리창 이론을 무의식적으로 실행하기 쉽다. 적은 임시방편 작업과 적은 기술 부채로 프로젝트를 깨끗하게 시작하는 것이 중요하다. 임시방편 작업이 슬며시 생기면 깨진 유리창처럼 동작하며 더 많은 임시방편 작업을 유발한다.

소프트웨어 프로젝트는 많은 비용과 오랜기간의 노력이 필요하다. 그리고 깨진 유리창을 궁지에 몰리게 하는 것은 소프트웨어 개발자로서 더 많은 책임감을 느끼게 한다. 우리는 프로젝트를 완료하지 못하고 다른 누군가에게 넘겨줘야 할 수도 있다. <u>그것은 그들과는 관계없는 레거시 코드이며 깨진 유리창을 만드는 한계점을 낮추게 된다.</u>

그러나 작업하는 코드의 일부가 전반적으로 중요하지 않거나 프로토타이핑이나 경제적인 이유 때문에 임시방편으로 작업을 하는 경우도 있다.

그렇게 의식적으로 추가된 응급처치를 문서화하는데 신경써야 한다. 이는 Michael Nygard가 그의 블로그(http://thinkrelevance.com/blog/2011/11/15/documenting-architecture-decisions)에서 제안한 Architecture Decision Records(ADRs) 공식문서에서 볼 수 있다. 우리 자신과 후임자에 빚지고 있는 것이다. 팀은 응급처치가 의식적, 좋은 이유로 취해야 한다는 것을 알기 때문에 팀의 모든 구성원은 이 문서를 인지하고 있고 깨진 유리창 효과를 줄일 것이다.

다음 부분은 이 책의 나타난 헥사고날 아키텍처의 응급처치라고 생각되는 패턴을 논의해볼 것이다. 응급처치의 효과와 논쟁거리, 그 내용에 대해 찬성하거나 반대하는 의견들을 살펴볼 것이다.



## 유스케이스간의 모델 공유

*4장(Implementing a Use Case)*에서 각 유스케이스에서는 입력 파라미터 유형과 리턴값의 유형은 달라야 한다고 의미하는 각각의 입/출력 모델을 가져야 한다고 이야기했다.
...
이 상황에서 모델을 공유하는 영향도는 **SendMoneyUseCase**와 **RevokeActivityUseCase**가 서로 결합되어 있다는 것을 나타낸다. **SendMoneyCommand** 클래스를 수정하면 양쪽 케이스 모두 영향받는다. 단일 책임 원칙에 의해 변경해야 할 이유를 공유한다. 양쪽 유스케이스가 동일 출력모델을 공유할 때도 동일하다.

유스케이스 간 입/출력 모델을 공유하는 것은 유스케이스가 기능적으로 경계가 있다면 가능하다. 즉, 어떤 요구사항을 공유한다면. 이 경우에는 *실제 특정 상세 부분을 수정하면 양쪽 유스케이스가 영향을 받기 원한다.*

두개의 유스케이스가 서로 독립적으로 변경되어야 한다면, 이것은 응급처치이다. 시작할때는 같은것 처럼 보여서 입/출력 중복처럼 느껴질지 모르겠지만 이런 경우에는 시작할 때 부터 유스케이스를 분리해야만 한다.

그래서 비슷한 개념의 여러 개의 유스케이스를 만들 때, 유스케이스가 각각 다르게 변경될 것인지에 대해 주기적으로 의문을 제기하는 것은 의미있는 일이다. 대답이 "그렇다"라면 입/출력 모델을 분리할 때다.
