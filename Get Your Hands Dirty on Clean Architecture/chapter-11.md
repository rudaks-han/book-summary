...
이 책의 첫부분에서, 나는 편법으로 처리함으로 인해서 많은 기술부채를 만들어내고 결코 수정되지 않는다는 사실을 비난했었다.

편법으로 처리하지 않기 위해서는 그 내용을 인지하고 있어야 한다. 그래서 이 장의 목표는 잠재가능성이 있는 편법으로 처리하는 것에 대한 의식을 높이고 그 효과를 논의해보는 것이다.

이러한 정보를 통해서 의도치 않게 편법으로 처리하는 것을 인지하고 고쳐 나갈 수 있다. 혹은 그럴게 할 수 밖에 없는 경우라면, 우리는 편법을 의도적으로 선택할 수도 있다. 

건축공학 도서나 혹은 더욱 끔찍하겠지만 항공기술관련 도서에서 이런 편법이 있다고 생각해보자. 하지만 우리 대부분은 고층건물이나 항공기 관련 소프트웨어를 만들지는 않는다. 소프트웨어는 하드웨어보다 더 부드러워야 하고 변경이 쉬워야 한다. 그래서 우선은 편법으로 처리하고 나중에(결코 하지 않겠지만) 수정하는 것이 더 경제적일 수도 있다.
...
* 편법으로 처리된 로직이 많은 코드로 작업할 때 편법을 사용하는 코드가 추가될 가능성은 높아진다.
...
코드로 작업하는 것이 차를 부수는 것과 같지는 않지만 우리는 깨진 유리창 이론을 무의식적으로 실행하기 쉽다. 편법을 줄이고 기술 부채를 가능한 만들지 않고 프로젝트를 깨끗하게 시작하는 것이 중요하다. 편법이 슬며시 생기기 시작하면 깨진 유리창처럼 더 많은 편법을 유발한다.
