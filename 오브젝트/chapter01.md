...
* 로버트 L.글래스는 <<소프트웨어 크리에이티비티 2.0>>에서 "이론 대 실무"라는 개인적인 의견을 밝히고 있다.
    ...
    * 글래스의 결론은 이론보다 실무가 먼저라는 것이다. 어떤 분야든 초기 단계에서는 아무것도 없는 상태에서 이론을 정릭하기 보다는 실무를 관찰한 결과를 바탕으로 이론을 정립하는 것이 최선이다.
* 실무는 훌륭한 소프트웨어를 설계하기 위해 필요한 다양한 기법과 도구를 초기부터 성공적으로 적용하고 발전시켜 왔던 것이다.
* 이론은 실문에서 반복적으로 적용되던 기법들을 이론화한 것들이 대부분이다.
* 설계 분야에선느 실무는 이론을 압도한다. 설계에 관해 설명할 때 가장 유용한 도구는 이론으로 덕지덕지 치장된 개념과 용어가 아니라 '코드' 그 자체다.
...
마팅에 따르면 모든 모듈은 제대로 실행돼야 하고, 변경이 용이해야 하며, 이해하기 쉬워야 한다.



### 예상을 빗나가는 코드

* 문제는 관람객과 판매원이 소극장의 통재를 받는 수동적인 존재라는 점이다.
...
현실에서는 관람객이 직접 자신의 가방에서 초대장을 꺼내 판매원에게 건낸다. 티켓을 구매하는 관람객은 가방 안에서 돈을 직접 꺼내 판매원에게 지불한다. 판매원은 매표소에 있는 키켓을 직접 꺼내 관람객에게 건내고 관람객에게서 직접 돈을 받아 매표소에 보관한다. 현재의 코드는 우리의 상식과는 너무나도 다르게 동작하기 때문에 코드를 읽는 사람과 제대로 의사소통하지 못한다.
...
* 이는 객체 사이의 의존성과 관련된 문제다. 
    ...
    * 의존성이라는 말 속에는 어떤 객체가 변경될 때 그 객체에게 의존하는 다른 객체도 함께 변경돌 수 있다는 사실이 내포돼 있다.
...
* 절자치향
    ...
    * 이 관점에서 Theater의 enter 메서드는 **프로세스**이며 Audience, TicketSeller, Bag, TicketOffice는 **데이터**다.
...
* 책임의 이동이란
    * 변경 전의 절차적 설계에서는 Theater가 전체적인 작업을 도맡아 처리했다. 
...
레베카 워프스브록은 능동적이고 자율적인 존재로 소프트웨어 객체를 설계하는 원칙을 가리켜 의인화(anthoropomorphism)라고 부른다.

* 관람객이 입장하기 위해서는 누군가가 문을 열고 입장을 허개해줘야 한다.
* 가방에서 돈을 꺼내는 것은 관람객이지 가방이 아니다.
* 판매원이 매표소에 없는데도 티켓이 저잘로 관람객에게 전달되지는 않을 것이다.

이해하기 쉽게 변경하기 쉬운 코드를 작성하고 싶다면 차라리 한 편의 애니메이션을 만든다고 생각하라. 다른 사람의 코드를 읽고 이해하는 동안에는 애니메이션을 보고 있다고 여러분의 뇌를 속여라.
