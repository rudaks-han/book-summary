...
우리에게 무엇을 말하느냐고? 대부분 200줄 정도 파일로더 커다란 시스템을 구축할 수 있다는 사실이다. 일반적으로 큰 파일보다 작은 파일이 이해하기 쉽다.
...
승수 사이에는 공백이 없다 .곱셉은 우선순위가 가장 높기 때문이다.
...
들여쓰기한 파일은 구조가 한눈에 들어온다. 반면, 들여쓰기 하지 않은 코드는 열심히 분석하지 않는한 거의 불가해하다.



들여쓰기 무시하기. 때로는 간단한 If문, 짧은 while문, 짧은 함수에서 들여쓰기 규칙을 무시하고픈 유횩이 생긴다. 
